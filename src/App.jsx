...
//import { addSectorData } from './addSectorData';
import { retrieveDataFromFirestore } from './retrieveSectorData';


const firebaseConfig = {
  apiKey: "AIzaSyCLhZ1qGO4Is18aERas7K95zu9HY-qU28s",
  authDomain: "super-duper-test-30507.firebaseapp.com",
  projectId: "super-duper-test-30507",
  storageBucket: "super-duper-test-30507.appspot.com",
  messagingSenderId: "172299776574",
  appId: "1:172299776574:web:191da3d4dc99278cda576f"
};
...
const App = () => {
  ...
  const[sectorDataArray, setSectorDataArray] = useState([])
  ...
  const saveData = (event) => {
    ...
    if(name !== '' && sector !== '' && agreed) {
      ...
      .then(() => {
        ...
        .then((doc) => {
          if (doc.exists) {
            ...
            const retrievedSector = doc.data().sector;
            const retrievedAgreeStatus = doc.data().agreed
            setName(retrievedName)
            setSector(retrievedSector)
            if(retrievedAgreeStatus) {
              toggleActiveClass()
            }
          } else {
      ...
      alert('Agree to terms before proceeding')
    }
  }
  ...
  const fetchData = async () => {
    const retrievedData = await retrieveDataFromFirestore(db);
    setSectorDataArray(retrievedData)
  }
  ...
  return (
    <div className='body-div'>
      ...
      <form onSubmit={saveData}>
        <div className='form-each-input-div'>
          <p className='input-overlap-paragraph'>Name:</p>
          <input type='text' name='name' value={name} onChange={(event) => setName(event.target.value)} required aria-label="Name Input Box"/>
        </div>
        <div className='form-each-input-div'>
          <p className='input-overlap-paragraph'>Sector:</p>
          <select name='sector' aria-label='Sector Select Input' aria-expanded="false" onChange={(event) => setSector(event.target.value)} required>
            {sectorDataArray.map((sector, sectorIndex) => (
              <React.Fragment key={sectorIndex}>
                <option value={`${sector.option}`}>
                  {sector.sector}
                </option>
                {sector.options.map((option, optionIndex) => (
                  <React.Fragment key={optionIndex}>
                    ...
                    {option.subOptions && option.subOptions.map((subOption, subOptionIndex) => (
                      <React.Fragment key={subOptionIndex}>
                        <option value={`${subOption.option}`} >
                        &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;{subOption.option}
                      </option>
                      {subOption.subOptions && subOption.subOptions.map((subOption, subOptionIndex) => (
                        <React.Fragment key={subOptionIndex}>
                          <option value={`${subOption.option}`}>
                            &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;{subOption.option}
                        </option>
        ...
        <div className='terms-check-div'>
          <div className="custom-check" id="customCheck">
            ...
            <label htmlFor="checkInput" className="check-label" role="checkbox" aria-checked="false" tabIndex="0" aria-label="Mark agree to terms as checked" onKeyDown={(event) => handleCheckKeyDown(event)}>
